const DEFAULT_COLUMNS = [
  ...
  'Filename', 'Picture URL', 'AI Source', 'Delete'
]
  ...
  if (visibleSaved) {
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        // Ensure newly introduced default columns appear for existing users.
        const insertBeforeDelete = (name) => {
          if (visibleColumns.includes(name)) return
          const deleteIdx = visibleColumns.indexOf('Delete')
          if (deleteIdx >= 0) visibleColumns.splice(deleteIdx, 0, name)
          else visibleColumns.push(name)
        }

        insertBeforeDelete('AI Source')
      }
    } catch (err) {
